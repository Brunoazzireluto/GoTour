...
A type assertation fornece acesso ao valor concreto subjacente de um valor de interface.
	t := i.(T)
Esta declaração afirma que o valor de interface i detém o tipo concreto T e atribui
o valor de T subjacente à variável t.

Se i não detém uma T, a declaração irá desencadear um erro pânico.

Para testar se um valor de interface possui um tipo específico,
uma _type assertation_ pode retornar dois valores: o valor subjacente e um valor
booleano que informa se a afirmação é correta.
	t, ok := i.(T)
